...
CVctLineManager::CVctLineManager()
{
	...
	m_colSum = 0;
	m_rowSum = 0;
	m_LinePts.InitBlock(0, 0, m_blkWid, m_blkHei);
...
void CVctLineManager::InitBlock(int x, int y, int cx, int cy, int dx, int dy)
{
	...
	m_LinePts.InitBlock(x, y, m_blkWid, m_blkHei);
	m_blkObjs.InitBlock(m_colSum, m_rowSum);
}
...
void CVctLineManager::DrawRectVct(double xgoff, double ygoff, double minx, double miny, double maxx, double maxy, float zoomRate)
{
	...
	double offx,offy; CLinkList<ObjLink>* link = NULL;
	if( m_bUseDispList )
	{
		...
		while( link )
		{
			...
			if( objSum && pList )
			{
				offx = c*m_blkWid + m_Origin.x - xgoff;
				offy = r*m_blkHei + m_Origin.y - ygoff;
				::glPushMatrix();
				::glTranslated( offx, offy, 0.0 );
