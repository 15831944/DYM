...
public:
	...
	//绘制当前范围的矢量
	void DrawRectVct(double xgoff, double ygoff, double minx, double miny, double maxx, double maxy, float zoomRate);

public:
	static CBlockVectors& GetInstance(HGLRC hRC);
	static bool	ShareDispList(HGLRC hRC);
    static void FreeInstance(HGLRC hRC);
...
public:
	...
	//绘制当前范围的矢量
	virtual void	DrawRectVct(double xgoff, double ygoff, double minx, double miny, double maxx, double maxy, float zoomRate)=0;
};
