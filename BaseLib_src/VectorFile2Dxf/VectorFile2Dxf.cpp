...
// 地物为NULL,进行修改NoStandard_0，by liukunbo
static void nulltoNoStandard(CString& strlayname)
{
	CString strbase = strlayname;
	CString strNull = strbase.Left(strbase.GetLength() - 2);//去掉最后二位
	if (strNull.GetLength()<1) //[11/1/2017 %jobs%]层码为空null
	{
		return;
	}
	if (strNull == "(null)" || strNull == "(NULL)" || strNull == "null" || strNull == "NULL") {
		strlayname = "NoStandard_0";
	}
}
	...
	try
	{
		...
		//10%的进度条
		{
			...
			for(UINT i=0; i<layList.GetSize(); i++ )
			{
				...
				nulltoNoStandard(strLayName); //为null的时候进行修改
