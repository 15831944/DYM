...
void CLineBlockCut::InitBlock(int dx, int dy)
{
	m_blkWid = dx>0?dx:1; 
	m_blkHei = dy>0?dy:1;
}
...
void CLineBlockCut::RegPoint(double x, double y, float sz)
{
	...
	int col = (int)floor(x/m_blkWid); 
	int row = (int)floor(y/m_blkHei); 
...
void CLineBlockCut::RegisterLine(const PtInfo& curPt)
{
	...
	int cmin = (int)floor(x0/m_blkWid);
	int cmax = (int)floor(x1/m_blkWid)+1;
	int rmin = (int)floor(y0/m_blkHei);
	int rmax = (int)floor(y1/m_blkHei)+1;
