...
CSpGLSteView::CScrollAssist::CScrollAssist()
{
	m_pWnd = NULL;
	m_zoom = 1.0;
...
// zoom : real zoomrate
// CENTER ZOOM
void CSpGLSteView::CScrollAssist::Change(int cx,int cy,CSize size,float zoom)
{
	int x_cen,y_cen;
...
// zoom : real zoomrate
// RECTANGLE ZOOM
void CSpGLSteView::CScrollAssist::Change(CPoint centerPoint,CSize clientSize,CSize imageSize,float zoom)
{
...
bool CSpGLSteView::CScrollAssist::OnVScroll(UINT nSBCode,UINT nPos) // if handled, return true
{
	switch( nSBCode )
	{
	//case SB_THUMBTRACK:
...
bool CSpGLSteView::CScrollAssist::OnHScroll(UINT nSBCode, UINT nPos)	// if handled, return true
{
	switch( nSBCode )
	{
	//case SB_THUMBTRACK:
...
void CSpGLSteView::CScrollAssist::OnCreate(CWnd *pwnd)
{
	m_pWnd = pwnd;
	m_zoom = 1.0;
	m_HPosHi = 0; m_HPosLow = 0;
...
void CSpGLSteView::CScrollAssist::ResetScroll()
{
	m_offx = m_offy = 0;
...
int CSpGLSteView::CScrollAssist::GetScrollPos(int nBar)
{	
	if( nBar==SB_HORZ ) 
...
void CSpGLSteView::CScrollAssist::SetBothPos(int x,int y)
{
	m_HPosHi  = x/10;
	m_VPosHi  = y/10;
...
void CSpGLSteView::CScrollAssist::SetScrollRange(int nBar, int Min, int Max, bool bRedraw)
{
	if( Max<=Min ) 
	{
...
void CSpGLSteView::CScrollAssist::SetStep(int step)
{
	m_ScrollStep = step/10;
}

short CSpGLSteView::CScrollAssist::Undo(float* zoom)
{
...
int	CSpGLSteView::CScrollAssist::GetUndoSum()
{ 
	return m_curUndo; 
}

CSize CSpGLSteView::CScrollAssist::GetScrollRange()
{ 
	return m_ScrollRange; 
}

void CSpGLSteView::CScrollAssist::SetLastOP()
{
...
void CSpGLSteView::CScrollAssist::ClearUndo()
{ 
	m_curUndo = 0; 
}