...
void CJobQueue::ProjFunc(const JOBNODE& job)
{
    ...
    if( buf ) {
        ...
        int step = m_nBlockSize/8;
        for( int j=1; j<8; ++j )
        {
            pMemDC->MoveTo( step*j, step ); pMemDC->LineTo( step*j, m_nBlockSize-step );
            pMemDC->MoveTo( step, step*j ); pMemDC->LineTo( m_nBlockSize-step, step*j );
        }
        ...
        //贴到屏幕左上角看看(BGR方式显示)
        pScrn->BitBlt( 0, 512, m_nBlockSize, m_nBlockSize, pMemDC, 0, 0, SRCCOPY);
