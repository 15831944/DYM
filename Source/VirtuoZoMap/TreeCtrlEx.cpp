...
{
	BOOL ret = CTreeCtrl::SetCheck(hItem, fCheck);
	CWnd *pWnd = GetParent();
	if (pWnd) pWnd->SendMessage(WM_CHECK_CHANGE_CMD, LPARAM(hItem), LPARAM(fCheck));
	return ret;
}
/////////////////////////////////////////////////////////////////////////////    
...
{     
	...
	if( (nFlags & (MK_CONTROL|MK_SHIFT)) == 0 &&
		(GetStyle() & TVS_EDITLABELS) != 0 && 
		(nHitFlags & TVHT_ONITEMLABEL) != 0 ) 
	{
    ...
    if( nHitFlags & TVHT_ONITEM )
	{
		...
		{
			SetCheck( hClickedItem, !GetCheck(hClickedItem) );
			return;
		}
	...
	{
		if( !(nHitFlags & TVHT_ONITEMBUTTON) && !(nFlags & MK_CONTROL) ){
			//不调用树控件的默认函数，修改成取消所有项的选中状态
			for( HTREEITEM hItem = GetRootItem(); hItem!=NULL; hItem = GetNextVisibleItem(hItem) )   
				if( GetItemState(hItem,TVIS_SELECTED) & TVIS_SELECTED )
					SetItemState(hItem,0,TVIS_SELECTED);
			if( GetSelectedItem() ) SelectItem(NULL);
		}else
...
{   
    ...
    m_bEditLabelPending = FALSE; 
    CTreeCtrl::OnLButtonDblClk(nFlags, point);   //by tjhuang[2013-12-11],取消双击自动选中item
}   
