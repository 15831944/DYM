...
{
	//pDC->SetBkMode(TRANSPARENT);//ÉèÖÃ×ÖÌå±³¾°ÑÕÉ«
...
// 				OnDrawBorder(pDC, &itemRect);
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | m_byTextAlign | DT_CALCRECT);
		int left = 0;
		...
		case DT_LEFT:
			...
			break;
		case DT_CENTER:
			left = (centerRect.Width() - captionRect.Width()) / 2;
			break;
		case DT_RIGHT:
			left = centerRect.Width() - captionRect.Width();
			break;
		default:
			break;
		}
		captionRect.OffsetRect(left, (centerRect.Height() - captionRect.Height())/2);
		/* RFU
		...
		*/
		if (m_bIsDisabled)
		{
			...
			pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | m_byTextAlign);
			captionRect.OffsetRect(-1, -1);
...
DWORD CButtonST::SetTextAlign(BYTE byAligin)
{
	m_byTextAlign = byAligin;
	switch (byAligin)
	{
	case DT_LEFT:
		m_byTextAlign = DT_LEFT;
		break;
	case DT_RIGHT:
		m_byTextAlign = DT_RIGHT;
		break;
	default:
		m_byTextAlign = DT_CENTER;
		break;
	}
	return 0;
}
