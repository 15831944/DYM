...
// VirtuoZoMapView.cpp : CSpVctView 类的实现
//
...
IMPLEMENT_DYNCREATE(CSpVctView, CSpGLImgView)

BEGIN_MESSAGE_MAP(CSpVctView, CSpGLImgView)
	// 标准打印命令
	...
	ON_COMMAND(ID_VIEW_FIT_SCALE, &CSpVctView::OnZoomFit)
	ON_COMMAND(ID_VIEW_SCALE_1V1, &CSpGLImgView::OnZoomNone)
	ON_COMMAND(ID_VIEW_SCALE_1V2, &CSpVctView::OnViewScale1v2)
	ON_COMMAND(ID_VIEW_SCALE_2V1, &CSpVctView::OnViewScale2v1)
	ON_COMMAND(ID_VIEW_SCALE_1VN, &CSpVctView::OnViewScale1vn)
	ON_COMMAND(ID_VIEW_SCALE_NV1, &CSpVctView::OnViewScaleNv1)
	ON_COMMAND(ID_VIEW_UNDO_ZONE, &CSpGLImgView::OnZoomUndo)
	ON_MESSAGE(WM_INPUT_MSG, &CSpVctView::OnInputMsg)
	ON_MESSAGE(WM_OUTPUT_MSG, &CSpVctView::OnOutputMsg)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_RBUTTONDOWN()
	ON_WM_MOUSEMOVE()
	...
	ON_WM_MBUTTONDOWN()
	ON_WM_MBUTTONUP()
	ON_COMMAND(ID_SET_CUR_COORD, &CSpVctView::OnSetCurCoord)
	ON_CONTROL(EN_SETFOCUS,ID_INDICATOR_COORD, &CSpVctView::OnSetCurCoord)
	ON_CONTROL(EN_SETFOCUS,ID_INDICATOR_HIGHTRATE, &CSpVctView::OnSetCurHight) //  [10/30/2017 %jobs%]
	ON_COMMAND(ID_SET_CUR_Z, &CSpVctView::OnSetCurZ)
	ON_WM_KILLFOCUS()
	ON_COMMAND(ID_SET_ZVALUE_STEP, &CSpVctView::OnSetZvalueStep)		//Add [2013-12-30]
	ON_COMMAND(ID_CURSOR_CENTER, &CSpVctView::OnCursorCenter)			//Add [2014-1-2]
	ON_UPDATE_COMMAND_UI(ID_CURSOR_CENTER, &CSpVctView::OnUpdateCursorCenter)			//Add [2014-1-2]
END_MESSAGE_MAP()

// CSpVctView 构造/析构
extern CMainFrame* pMFrame; 
CSpVctView::CSpVctView()
{
	SetMargin(256); 
	m_bZoomCent = true; 
	m_bShareVct = true;
...
void CSpVctView::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{	// storing code
		ar << GetZoomRate();
...
*/
BOOL CSpVctView::PreTranslateMessage(MSG* pMsg)  
{
		...
		case VK_TAB:
			if (::GetKeyState(VK_CONTROL) < 0)
			{
				try
				{
					...
					CSpSteView *pSteView = (CSpSteView*)theApp.GetView(1);
					if (pSteView)
					{
						pSteView->SetCursorGrdPos(gPos, false, false);
						pSteView->MoveCursor2CrossPos();
					}
				}
...
void CSpVctView::AutoDrawMoveCursor(GPoint gpt)
{
...
void CSpVctView::AutoPanView(GPoint gpt1, GPoint gpt2)
{
...
void CSpVctView::AutoZoomView(short zDelta)
{
...
//4.返回最小距离点的索引
int CSpVctView::polygon(GPoint curPoint) //  [6/15/2017 jobs]
{ 
...
// CSpVctView 消息处理程序
//extern CMainFrame* pMFrame; 
void CSpVctView::OnInitialUpdate()
{
	CMainFrame* pMFrame=(CMainFrame*)AfxGetMainWnd();
...
void CSpVctView::GLDrawLine(const double* buf, int elesum, COLORREF col)
{
...
void CSpVctView::GLDrawDragVectors()
{
	CVirtuoZoMapDoc *pDoc = (CVirtuoZoMapDoc *)GetDocument(); ASSERT(pDoc);
...
void CSpVctView::GLDrawMark(GPoint* AryGpt, UINT sum, double size, int Type, COLORREF col)
{
...
void CSpVctView::GLDrawValidRect()
{
...
void CSpVctView::GLDrawCursor(double x, double y)
{
...
BOOL CSpVctView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	CVirtuoZoMapDoc *pDoc = (CVirtuoZoMapDoc *)GetDocument();
	switch( nHitTest )
	{
...
LRESULT CSpVctView::OnInputMsg( WPARAM wParam,LPARAM lParam )
{
	CVirtuoZoMapDoc *pDoc = (CVirtuoZoMapDoc *)GetDocument(); ASSERT(pDoc);
	double size = pDoc->m_dlgOptimize.m_MarkSize/GetZoomRate(); 
	double snapsize = pDoc->m_nSnapsize/GetZoomRate();
...
LRESULT CSpVctView::OnOutputMsg(WPARAM wParam, LPARAM lParam)
{
	try
	{
		switch(wParam)
...
void CSpVctView::OnLButtonDown(UINT nFlags, CPoint point)
{
	m_LastLBDown = point;
// 	CString sss;
...
void CSpVctView::OnLButtonUp(UINT nFlags, CPoint point)
{
...
void CSpVctView::OnRButtonDown(UINT nFlags, CPoint point)
{
...
void CSpVctView::OnMouseMove(UINT nFlags, CPoint point)
{
...
void CSpVctView::OnMButtonDown(UINT nFlags, CPoint point)
{
	m_tlStat = stMOVE; 
	m_LbtPos = point; 
	SetCapture();
...
void CSpVctView::OnMButtonUp(UINT nFlags, CPoint point)
{
...
void CSpVctView::OnTimer(UINT_PTR nIDEvent)
{
	switch(nIDEvent)
	{
	case OnTimerSnapIdx:
...
void CSpVctView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	try
	{
...
void CSpVctView::SetCursorGrdPos(GPoint gpt)
{
	double x = gpt.x, y = gpt.y;
	GrdToImg(&x, &y); ImgToClnt(&x, &y);
	CRect rect; GetClientRect(&rect);
...
void CSpVctView::InputVector(OutPutParam* ptrvctobj)
{
...
void CSpVctView::InputLineObj(CGrowSelfAryPtr<double> *DesObj, LINEOBJ *resobj)
{
...
void CSpVctView::RightAngleHelp(GPoint & gPt)
{
...
BOOL CSpVctView::OperMsgToMgr(UINT nFlags, GPoint gpt, OperSvr opersvr, CView* pView)
{
	CVirtuoZoMapDoc *pDoc = (CVirtuoZoMapDoc *)GetDocument();
...
void  CSpVctView::CPtLineToDBLine(CPoint spt, CPoint ept, CGrowSelfAryPtr<double> *line)
{
...
void    CSpVctView::RotatoCoor(double lfRadian)
{
	Rect3D rect2pt;
...
void   CSpVctView::CPointToGPoint(CPoint cpt, GPoint &gpt, double lfHeight)
{
...
void CSpVctView::ResetViewInfo(double kap, Rect3D rect2pt, bool bMoveToRectCent)
{
...
void CSpVctView::SetValidRect(CPoint scpt, CPoint ecpt)
{
...
void CSpVctView::ScrollClnt(int x, int y)
{
	CSpGLImgView::ScrollClnt(x, y);
	UpdateAerial();
}

void CSpVctView::ZoomCustom(CPoint point, float zoomRate)
{
...
void CSpVctView::OnViewScale1v2()
{
	CRect rc;GetClientRect(&rc); 
	CSpGLImgView::ZoomCustom(rc.CenterPoint(),0.5f);
}

void CSpVctView::OnViewScale2v1()
{
	CRect rc;GetClientRect(&rc); 
...
BOOL  CALLBACK WuJiScanVct1vn(BOOL bCancel,LPCTSTR strScanPara, void *pClass)
{
	...
	if ( count == 1 )
	{
		CSpVctView *pView = (CSpVctView *)pClass;
		retrn = TRUE; 
...
void CSpVctView::OnViewScale1vn()
{	
	try
	{
...
void CSpVctView::OnViewScaleNv1()
{	
	try
	{
		TCHAR strmsg[128];  ZeroMemory(strmsg, sizeof(strmsg)); CString strtable;
...
void CSpVctView::OnSetFocus(CWnd* pOldWnd)
{
...
void CSpVctView::OnUpdateIndicatorZoomRate(CCmdUI* pCmdUI)
{
...
void CSpVctView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	switch(lHint)
	{
	case hc_SetLayState:
...
void CSpVctView::OnSetCurHight() //  [10/30/2017 %jobs%]
{
...
void CSpVctView::OnKillFocus(CWnd* pNewWnd)
{
...
void CSpVctView::OnSetZvalueStep()
{
	// TODO: 在此添加命令处理程序代码
	try
	{
...
void CSpVctView::OnCursorCenter()			//Add [2014-1-2]
{
	// TODO: 在此添加命令处理程序代码
...
void CSpVctView::OnUpdateCursorCenter(CCmdUI *pCmdUI)			//Add [2014-1-2]
{
