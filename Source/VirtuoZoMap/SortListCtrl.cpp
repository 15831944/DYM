// SortListCtrl.cpp : implementation of the CSortListCtrl class
//
...
{
    if( GetHeaderCtrl()==NULL ) return -1;
    if(m_iNumColumns==0) m_iNumColumns = GetHeaderCtrl()->GetItemCount();
    if(m_iNumColumns==0) return -1;
    ...
    memset( clrText, 0, sizeof(COLORREF)*m_iNumColumns );
	COLORREF * clrBak= new COLORREF[ m_iNumColumns ];
    memset( clrBak, 0xff, sizeof(COLORREF)*m_iNumColumns );
...
int CSortListCtrl::AddItemColor(LPCTSTR pszText, COLORREF crText, COLORREF crBak)
{
	...
	const int iIndex = InsertItem( GetItemCount(), pszText );
	ItemData *m_pSortItemData=new ItemData[GetHeaderCtrl()->GetItemCount()];
/*
...
BOOL CSortListCtrl::GetSubItemRect(int nItem, int nSubItem, int nArea, CRect &rect)
{
	...
	if ((nItem < 0) || nItem >= GetItemCount())
		return FALSE;
	ASSERT(nSubItem >= 0);
    int itemCount = GetHeaderCtrl()->GetItemCount();
	ASSERT(nSubItem < itemCount);
	if ((nSubItem < 0) || nSubItem >= itemCount)
		return FALSE;
