...
BOOL CMapSvrFileAct::ImportContolPointFile(CSpVectorFile* pFile,LPCTSTR lpSourceFile,int curfileIdx,CGrowSelfAryPtr< CSpVectorObj* >* pObjList) 
{
	...
	//ctl控制点格式
	if (dlg.m_strCtlPtPath.Find(".ctl") != -1) {
		//总点数
	...
	else if (dlg.m_strCtlPtPath.Find(".txt") != -1) {
...
BOOL CMapSvrFileAct::Exprot2CurFile(VctFileType eft)
{
	...
	case File_VVT:	//文本文件	//Add [2013-12-24]
		...
		break;
	case  File_CASS: //  [6/4/2018 jobs]
		{
			Export2CASS(pFile,curfileIdx);
		}
		break;
	default:
		return FALSE;
		...
		if(dlg.m_bCut)
		{
			...
			else
			{
				...
				for (UINT i=0; i<sum; i++)
				{
					...
					try
					{
						VectorFile2Dxf(pFile,m_pSymMgr,strDxf,strLayPath,dlg.m_bDimension,dlg.m_bSpline,dlg.m_bPoint,dlg.m_bSymbolized,dlg.m_bFill,&rect2Pt,m_hWndRec,m_msgID);
					}
					catch (CException* e)
					{
						e->ReportError();
